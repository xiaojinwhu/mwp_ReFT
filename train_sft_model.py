...
from src.utils import compute_ETA, floatify, set_seed
...
def prepare_datasets_and_data_loaders(args, tokenizer):
    with accelerator.main_process_first():
        ...
        src_name = raw_dataset["train"][0]["item_id"].split("_")[
            0
            ...
            wandb.config.update(
                {
                    ...
                    "train_input_ids_max_length": max(
                        tokenized_dataset["train"]["input_ids_max_length"]
                    ),
                    "test_input_ids_max_length": max(
                        tokenized_dataset["test"]["input_ids_max_length"]
                    ),
                }
            )
        ...
        for idx, batch in t:
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    ] > best_eval_log_dict.get("Eval.Gen.value_accuracy_best", 0):
                        is_best = True
                        best_eval_log_dict["Eval.Gen.value_accuracy_best"] = (
...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args["model_name_or_path"], low_cpu_mem_usage=True, torch_dtype=torch.bfloat16
    )
    ...
    num_training_steps = (
        len(train_dataloader) // accelerator.num_processes * n_epochs
        ...
        for epoch in t:
            ...
            if evaluating_epoch_freq is not None and epoch % evaluating_epoch_freq == 0:
                ...
                if eval_log_dict["Eval.Gen.value_accuracy"] > best_eval_log_dict.get(
                    "Eval.Gen.value_accuracy_best", 0
                ):
                    is_best = True
